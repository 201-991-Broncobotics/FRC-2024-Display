...
class Polygon(object):
    ...
    def setPos(self, pos: tuple[float, float]):
        self.x = pos[0]
        self.y = self.surface.get_height() - pos[1]

        self.pivotPoint = (self.x + self.scale * self.pivot[0] * math.cos(self.angle) + self.scale * self.pivot[1] * math.sin(self.angle),
                           self.surface.get_height() - (self.y + self.scale * self.pivot[0] * math.sin(self.angle) - self.scale * self.pivot[1] * math.cos(self.angle)))
